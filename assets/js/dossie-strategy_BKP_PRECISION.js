...
const DossieStrategy = {
    ...
    updateQuickKPIs: function(r, orig, scenario, delta) {
        ...
        const imovelRef = orig.results.fullResults.valorImovel || 250000;
        if (pCobertura) pCobertura.innerText = Math.min(100, Math.round((r.subsidio + scenario.fgts + scenario.entrada) / imovelRef * 100)) + '%';
    },
    ...
    updateHeaderDeficitBadge: function(delta) {
        ...
        if (stratBadge && stratVal) {
            ...
            if (delta > 1000) {
                ...
                stratVal.innerText = `DÉFICIT: ${window.MT_Utils.formatCurrency(delta)}`;
            } else {
