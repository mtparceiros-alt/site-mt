    ...
    if (btnDownload) {
        btnDownload.addEventListener('click', async function () {
            ...
            try {
                ...
                if (wsInicio) {
                    wsInicio.getCell('C22').value = d.nome || 'Cliente MT';
                    wsInicio.getCell('B34').value = d.parcelaEntrada || 0;
                    wsInicio.getCell('E34').value = d.evolucaoMedia || 0;
                    wsInicio.getCell('G34').value = d.parcelaPosChaves || 0;
                }
                ...
                if (wsSimul) {
                    ...
                    wsSimul.getCell('H13').value = d.renda || 0;
                    wsSimul.getCell('H26').value = d.fgts || 0;
                    wsSimul.getCell('H28').value = d.entrada || 0;
                    wsSimul.getCell('H30').value = d.carteira ? 'SIM' : 'NÃO';
                    wsSimul.getCell('H17').value = d.dividas || 0;
                ...
                if (wsLaudo) {
                    wsLaudo.getCell('C9').value = d.renda || 0;
                    wsLaudo.getCell('C13').value = d.potencial || 0;
                    wsLaudo.getCell('C17').value = d.subsidio || 0;
                    wsLaudo.getCell('C21').value = d.fgts || 0;

                    wsLaudo.getCell('F9').value = d.poder || 0;
                    wsLaudo.getCell('F13').value = d.renda || 0;
                    wsLaudo.getCell('F17').value = d.parcelaPosChaves || 0;

                    // Lógica de Aprovação
                    const comprometimento = (d.parcelaPosChaves + (d.dividas || 0));
                    const isAprovado = (d.renda > 0) && (comprometimento <= (d.renda * 0.35));
                    wsLaudo.getCell('H17').value = isAprovado ? "PRÉ-APROVADO" : (d.renda > 0 ? "ALERTA DA PARCELA" : "DADOS INCOMPLETOS");

                    // Lógica do Score IA MT
                    ...
                    wsLaudo.getCell('B23').value = score;
                }
