...
const DossieController = {
    ...
    rankEliteMatches: function(poderCompra) {
        ...
        // 2. Separar em grupos
        const matches = sorted.filter(e => parsePreco(e.preco) <= poderCompra);
        const deficits = sorted.filter(e => parsePreco(e.preco) > poderCompra);
    ...
    hydrateFromCache: function(preserveName = null) {
        ...
        try {
            ...
            const mappings = {
                ...
                'val-parcela-entrada': res.fullResults.parcelaEntrada > 0 ? `${res.fullResults.prazoObra || 36}x de ${formatter(res.fullResults.parcelaEntrada)}` : "100% Coberta",
                'val-evolucao-obra': formatter(res.parcela * 0.3),
                'val-pos-chaves': formatter(res.parcela),
                'val-timeline-obra': formatter(res.fullResults.parcelaEntrada + (res.parcela * 0.3)),
                'val-timeline-chaves': formatter(res.parcela),
                'viability-enquadramento': res.fullResults.foraDoMCMV ? "SBPE / Mercado" : (res.fullResults.faixaMCMV + " / Econômico")
            };

            // Atualização de ITBI
            const itbiEl = document.getElementById('viability-itbi-status');
            if (itbiEl) {
                if (res.fullResults.itbi === 0) {
                    ...
                    itbiEl.innerHTML = `<span class="material-symbols-outlined text-xs">payments</span> ITBI: ${formatter(res.fullResults.itbi)}`;
    ...
    recalculateForProperty: function(empName) {
        ...
        try {
            ...
            const extraRenda = _strategyOverrides.extraRenda || 0;
            const extraEntrada = _strategyOverrides.extraEntrada || 0;
            ...
            const mainEval = DossieEngine.evaluateMatch(precoImovel, scenario, true, window.MT_SimulatedPower, _strategyOverrides);
            const matchDelta = mainEval.delta;

            const stratEval = DossieEngine.evaluateMatch(precoImovel, scenario, false, 0, _strategyOverrides);
            const stratDelta = stratEval.delta;
            const r = stratEval.sim;

            window.lastCalculatedPoder = r.poderReal;
            
            // Renderizações de UI
            this.updateBentoResults(orig, precoImovel, formatter);
            this.updateStrategyPanelHeader(emp, precoImovel, stratDelta, formatter);
            this.updateProfileRuler(orig, formatter);
            this.updateStatusBadges(matchDelta, formatter);
    ...
    updateFluxoButtonState: function(matchDelta, formatter) {
        ...
        if (fluxoBtn) {
            ...
            if (matchDelta <= 1000) {
                ...
                if (statusLabel) {
                    statusLabel.innerHTML = '<span class="material-symbols-outlined text-[10px]">auto_awesome</span> CONDIÇÃO IDEAL';
                ...
                if (statusLabel) {
                    statusLabel.innerHTML = '<span class="material-symbols-outlined text-[10px]">verified_user</span> Liberado: Ver Cronograma';
    ...
    updatePaymentFlow: function(precoImovel, r, mesesObra, formatter) {
        if (window.DossieFluxo) {
            const finalFgts = (_strategyProfile.fgts || 0) + (_originalSimData.raw.fgts || 0);
            const extraEntrada = _strategyOverrides.extraEntrada || 0;
