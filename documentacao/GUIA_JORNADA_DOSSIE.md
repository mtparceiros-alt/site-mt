...
**Como Alterar Somente o Layout Mobile**

 - **Arquivos relevantes**: edite apenas o CSS para mudanças de layout móvel — `assets/css/dossie-v5.css`.
 - **Onde alterar o texto/estado do selo**: as strings (por exemplo, "Aguardando Imóvel", "CONDIÇÃO IDEAL") ficam em `assets/js/dossie-controller.js` — procure por `updateFluxoButtonState`.

Passo a passo rápido para ajustar somente o layout mobile

 1. Abra [assets/css/dossie-v5.css](assets/css/dossie-v5.css) e localize a seção `@media (max-width: 480px)` comentada como "MOBILE LAYOUT OVERRIDE".
 2. Para centralizar o selo e a seta sob o texto, ajuste estes seletores:
	 - `#btn-abrir-fluxo > .flex` (defina `flex-direction: column; align-items: center; gap`)
	 - `#btn-abrir-fluxo > .flex > .flex.flex-col` (defina `width: 100%; align-items: center`)
	 - `#fluxo-status-label` (ajuste `font-size`, `padding`, `white-space: nowrap` se quiser evitar quebra)
	 - `#timeline-chevron` (ajuste `margin: 0.4rem auto 0` para centralizar)
 3. Teste rápido: abra a página em `http://127.0.0.1:8000/dossie.html`, abra DevTools → Network → marque **Disable cache**, e recarregue com Ctrl+F5.
 4. Se tudo OK, propague as alterações para as cópias de distribuição (opcional):
	 - `dist/assets/css/dossie-v5.css` e `Site_MT_Final/assets/css/dossie-v5.css`
	 - Atualize o parâmetro de versão na tag `<link>` em [dossie.html](dossie.html) (ex: `?v=3`) para forçar cache-bust.

Exemplo de CSS que centraliza o selo/chevron (copiar/colar no final de `assets/css/dossie-v5.css`):

```css
@media (max-width: 480px) {
  #btn-abrir-fluxo > .flex { flex-direction: column !important; align-items: center !important; gap: 0.75rem !important; }
  #btn-abrir-fluxo > .flex > .flex.flex-col { width: 100% !important; align-items: center !important; }
  #fluxo-status-label { align-self: center !important; margin-left: 0 !important; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
  #timeline-chevron { display: block !important; margin: 0.4rem auto 0 !important; }
}
```

Boas práticas

- Mantenha apenas um bloco `@media (max-width: 480px)` para evitar regras conflitantes; coloque-o ao final do arquivo.
- Use `!important` com parcimônia — aqui usamos para garantir que o override mobile prevaleça sobre regras anteriores; remova quando a cascata estiver organizada.
- Prefira alterar `padding`, `gap` e `font-size` para ajustes visuais mínimos; altere strings apenas no controller JS.

Se quiser, eu posso propagar automaticamente este bloco de CSS para `dist/` e `Site_MT_Final/` e atualizar a referência em `dossie.html` após sua confirmação.
