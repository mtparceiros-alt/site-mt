...
## Anexo: Procedimento Rápido — Sincronizar `dist/` com o repositório monitorado pelo GitHub Desktop

Quando for necessário publicar uma build ofuscada/minificada gerada localmente (por exemplo para o fluxo de QA ou para subir via GitHub Desktop), siga estes passos. Este procedimento assume que o repositório de desenvolvimento principal está em `C:\Users\Marcos.PC_M1\Documents\site_mt` e que o repositório que o GitHub Desktop monitora está em `C:\Users\Marcos.PC_M1\Documents\github_site\Site_MT_Final`.

1. Gerar build ofuscada/minificada (no repositório principal):

```powershell
cd "C:\Users\Marcos.PC_M1\Documents\site_mt"
npm install    # somente se não houver node_modules atualizados
npm run build
```

2. Verificar o `dist/` gerado:

```powershell
dir .\dist -Recurse | Select-Object FullName, Length, LastWriteTime
```

3. Sincronizar (copiar) o conteúdo de `dist/` para a pasta monitorada pelo GitHub Desktop. O método seguro que usamos é o `robocopy` (Windows) — ele preserva a estrutura e sobrescreve arquivos alterados:

```powershell
robocopy "C:\Users\Marcos.PC_M1\Documents\site_mt\dist" "C:\Users\Marcos.PC_M1\Documents\github_site\Site_MT_Final" /E /IS /R:2 /W:5
```

4. Verificar o estado Git na pasta monitorada:

```powershell
cd "C:\Users\Marcos.PC_M1\Documents\github_site\Site_MT_Final"
git status --porcelain
```

Se aparecerem linhas (ex.: `M assets/js/simulator-v3-logic.min.js`), há mudanças prontas para commit.

5. Abrir o GitHub Desktop (ou forçar refresh) e confirmar que o repositório aberto é exatamente `C:\Users\Marcos.PC_M1\Documents\github_site\Site_MT_Final`:

- No GitHub Desktop: Repository → Show in Explorer (verifique o caminho).
- Se o app não exibir alterações, faça Repository → Refresh ou feche/abra o GitHub Desktop.

6. (Opção manual) Fazer commit/push via GitHub Desktop: selecione os arquivos alterados, escreva uma mensagem (ex.: "build: add generated dist from site_mt build") e clique em "Commit to master" e depois em "Push origin".

7. (Opção terminal) Se preferir que a máquina faça o commit e push automaticamente, os comandos são:

```powershell
git add -A
git commit -m "build: add generated dist from site_mt build"
git push origin master
```

Notas e recomendações:
- Confirme que você realmente quer sobrescrever o `Site_MT_Final` — este fluxo copia o resultado do `dist/` para esse repo, que pode ter histórico e remotes distintos.
- Se preferir evitar cópias manuais, adicione `C:\Users\Marcos.PC_M1\Documents\site_mt` ao GitHub Desktop (File → Add local repository). Assim você compila e comita direto no repo de origem.
- Esta seção descreve o fluxo que utilizei e testei localmente (inclui o `robocopy` exato usado). Guarde este procedimento como referência quando precisar repetir a operação.
